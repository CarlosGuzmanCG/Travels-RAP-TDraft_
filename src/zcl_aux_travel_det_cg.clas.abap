CLASS zcl_aux_travel_det_cg DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_travel_reported      TYPE TABLE FOR REPORTED z_i_travel_guz,
           tt_booking_reported     TYPE TABLE FOR REPORTED z_i_booking_guz,
           tt_supplements_reported TYPE TABLE FOR REPORTED z_i_booksuppl_guz.
...
CLASS zcl_aux_travel_det_cg IMPLEMENTATION.
...
*     Volvemos a agrupar por moneda. La agrupación la dejamos en LV_CURR
      LOOP AT GROUP lv_travel_key INTO DATA(ls_supplement_result)
                                   GROUP BY ls_supplement_result-currency INTO lv_curr.
